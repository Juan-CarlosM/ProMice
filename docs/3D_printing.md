...
!!! printer-3d-nozzle "3D printers"
    Resin printing is also a layer-by-layer printing technique. Cylindrical cavities and holes can suffer an ovalisation or deformations if they are printed horizontally. Therefore, the printing  orientation of the pieces will be based on the prioritisation of certain cavities and holes vertical position.
...
This ball-arm is divided into two parts; the larger section has a chamber for integrating what we call the [Yaw-lock system](mechanical_design.md#yaw_lock_system).
The rest is a sort of cover to complete the sphere. 
...
!!! info inline end "Info Ball-arm"
    The design of the piece and its dimensions are detailed in the [Mechanical design](mechanical_design.md) section.
    
From the image avobe we can notice the deformation on the resin piece due to the horizontal orientation of that cavity during printing. A post-printing craving will be necessary for the inner pieces to fit correctly. 

 
### The socket


The socket is also divided into two parts so that it is easy to assembled with the ball. A ring holds the two parts together.
...
Directly in the centre, above the spherical cavity, is the cavity for the ball joint [Hall sensor](sensors.md), which will measure the movement of the joint. This cavity is
...
There are four pulleys in total, each mounted on a motor. They transmit the motors.
